...
from agents.TransformerEstimator import BERTEmbedding, TransformerBlock
from utils.utils import remove_illegal

Transition = namedtuple('Transition', ['state', 'action', 'reward', 'next_state', 'done'])


class DQNTransformer(object):
    '''
...
class Estimator(object):
    ...
    def __init__(self, action_num=2, learning_rate=0.001, state_shape=None, mlp_layers=None, device=None):
        ...
        qnet = EstimatorNET(action_num, state_shape,
                            vocab_size=5,
                            hidden=512,
                            device=self.device)
        ...
        # set up optimizer
        self.optimizer = torch.optim.Adam(self.qnet.parameters(), lr=self.learning_rate,weight_decay=0.01)
...
class EstimatorNET(nn.Module):
    ...
    def __init__(self, action_num=2, state_shape=None,
                 vocab_size=7,
                 hidden=512,
                 n_layers=4, attn_heads=8,
                 dropout=0.1,
                 device=None):
        ...
        self.linear = torch.nn.Linear(self.hidden * 15, self.action_num)
        self.device = device
    ...
    def forward(self, s: torch.Tensor):
        ...
        size = s.size()
        batch_size = size[0]
        s = s.argmax(-2).flatten(start_dim=1)

        # [b,6,15]

        segment_info = [list(itertools.chain.from_iterable([[i for _ in range(15)] for i in range(6)])) \
                        for _ in range(size[0])]
        segment_info = torch.LongTensor(segment_info).to(self.device)
        mask = (s >= 0).unsqueeze(1).repeat(1, s.size(1), 1).unsqueeze(1)
        x = self.embedding(s, segment_info)
